...
$defaultPasswordHash = "$2y$10\$dLij/BtPMbPKtt/CxpzqVuSn1FBVq.es9spKQ87sdGVJmlu4J3zwq";

$user1 = new User('2@mail.com', 'Admin', null, $defaultPasswordHash);
$coordinatorUser = new User('coordinator@mail.com', 'Coordinator', null, $defaultPasswordHash);

$entityManager->persist($user1);
$entityManager->persist($coordinatorUser);

// Add student users ----------------------------------------------

$studentData = require_once (__DIR__ . '/student_data.php');
$studentNamesCount = count($studentData);
...
$partnerUsers = [];
$partnerData = require_once (__DIR__ . '/partner_data.php');
...
$lsegUser = $partnerUsers[0];
$googleUser = $partnerUsers[1];
$microsoftUser = $partnerUsers[2];
...
$microsoft = $organizations[0];
$google = $organizations[1];
$lseg = $organizations[2];

$microsoftUser->setOrganization($microsoft);
$googleUser->setOrganization($google);
$lsegUser->setOrganization($lseg);
...
$groupCoordinators->addUser($coordinatorUser);
...
foreach ($partnerUsers as $user) {
    $groupCyclePartnerAdmins->addUser($user);
}
